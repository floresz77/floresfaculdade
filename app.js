...
async function loginManager(){
  ...
  if(u==='GerusaSoares'&&p==='gerusa123'){mgrLogged=true;document.body.classList.add('gm');closeM('mo-glogin');showPage('manager');showToast('ðŸŸ£ Modo gerente ativado!');return;}
  showToast('â³ Verificando...');
  const{data}=await sb.from('pnm_users').select('data').eq('email',u).maybeSingle();
  if(!data||data.data.senha!==p){showToast('âŒ UsuÃ¡rio ou senha invÃ¡lidos!');document.getElementById('mg-p').value='';return;}
  const usr=data.data;
  if(usr.role!=='gerente'){showToast('âŒ Sem permissÃ£o de gerente!');return;}
  mgrLogged=true;document.body.classList.add('gm');curUser=usr;localStorage.setItem('pnm_user',JSON.stringify(usr));closeM('mo-glogin');showPage('manager');showToast('ðŸŸ£ Bem-vinda, '+usr.nome+'!');
}
...
async function loginUser(){
  ...
  if(curUser.role==='gerente'){mgrLogged=true;document.body.classList.add('gm');}
  showToast('ðŸ’• Bem-vinda, '+curUser.nome+'!');renderAcc();
}
